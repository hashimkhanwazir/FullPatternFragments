...
public class StarHandler extends AbstractRDFHandler {
     ...
     *
     */
    public StarHandler(ExecutorCompletionService<Boolean> executorCompletionService,
        ...
        this.processedStars = new HashSet<>();
    }

    private boolean isStarValid(List<Statement> sts) {
        boolean valid = true;
        for(Statement st : sts) {
            if(!isTripleValid(st)) valid = false;
        }
        return valid;
    }
    ...
    public void handleStatements(List<Statement> sts) throws RDFHandlerException {
        if(processedStars.contains(sts)) return;
        processedStars.add(sts);
