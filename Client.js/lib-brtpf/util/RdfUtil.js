...
util.matchesStarBinding = function (bindings, starPattern, boundStar) {
  ...
  for (var i = 0; i < starPattern.length; i++) {
    var tp = starPattern[i];
    if (this.isVariable(tp.predicate) && bindings[tp.predicate] !== undefined && bindings[tp.predicate] !== tp.predicate) return false;
    if (this.isVariable(tp.object) && bindings[tp.object] !== undefined && bindings[tp.object] !== tp.object) return false;
  }
  return true;
};
