...
//THIS FILE WAS EDITED FOR THE STAR PATTERN FRAGMENTS CLIENT BY CHRISTIAN AEBELOE <caebel@cs.aau.dk>.
...
function BufferedIterator(options) {
  ...
  var maxBufferSize = 30, autoStart = options.autoStart;
...
MultiTransformIterator.prototype._read = function (count, done) {
  ...
  while (source && !source.ended && transformerQueue.length < this._maxBufferSize) {
    ...
    if (current.length >= MAX_MPR) {
      // Create the transformer and listen to its events
      transformer = this._createTransformer(current) || new EmptyIterator();
      ...
      transformerQueue.push({ transformer: transformer, item: current });
  ...
  if (current.length > 0) {
    if (current.length === 1) {
      transformer = this._createTransformer(current[0]) || new EmptyIterator();
      transformer._destination = this;
      transformer.on('end',      destinationFillBuffer);
      transformer.on('readable', destinationFillBuffer);
      transformer.on('error',    destinationEmitError);
      transformerQueue.push({ transformer: transformer, item: current[0] });
    }
    else {
      transformer = this._createTransformer(current) || new EmptyIterator();
      transformer._destination = this;
