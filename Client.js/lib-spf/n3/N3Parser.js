...
// This file was modified by Christian Aebeloe <caebel@cs.aau.dk> for the creation of the
// Star Pattern Fragments client.
...
N3Parser.prototype = {
  ...
  _readSubject: function (token) {
    //todo Do readSubject, readPredicate, readObject and readPunctuation
    this._star = [];
  ...
  _readObject: function (token) {
    ...
    case 'literal':
      ...
      this._star.push({predicate : this._predicate, object : this._object});
      return this._readDataTypeOrLang;
    case '[':
      var bnode = '_:b' + blankNodeCount++;
      this._object = bnode;
      this._star.push({predicate : this._predicate, object : this._object});

      this._starPattern(this._subject, this._star,  this._graph);
      this._star = [];
      // Start a new triple with a new blank node as subject.
  ...
  _readPunctuation: function (token) {
    ...
    if (this._subject === null && !this._single) {
      if (!inversePredicate)
        this._starPattern(subject, this._star,  graph);
      else
        this._starPattern(object, this._star, graph);
    }
    return next;
  },
  ...
  // ### `_triple` emits a triple through the callback.
  _starPattern: function (subject, star, graph) {
    this._callback(null,
      { subject: subject, star: star, graph: graph || '' });
  },
