...
// Returns the Star Pattern Fragment for the given star pattern
FragmentsClient.prototype.getFragmentByStarPattern = function (pattern) {
  if (pattern.length === 1)
    return this.getFragmentByTriplePattern(pattern[0]);
  ...
  pattern.forEach(function (triplePattern) {
    ...
    var predicate = rdf.isVariableOrBlank(triplePattern.predicate) ? null : triplePattern.predicate;
    var object    = rdf.isVariableOrBlank(triplePattern.object)    ? null : triplePattern.object;

    if (rdf.isLiteral(predicate)) return fragment.empty();

    if (predicate !== null)
      starStr = starStr + 'p' + i + ',' + predicate + ';';
    if (object !== null)
      starStr = starStr + 'o' + i + ',' + object + ';';
    i++;
  });
  ...
  startFragment.getProperty('controls', function (controls) {
    // Replace all variables and blanks in the pattern by `null`
    var subject = rdf.isVariableOrBlank(pattern[0].subject)   ? null : pattern[0].subject;

    // Only attempt to fetch the fragment if its components are valid
    if (rdf.isLiteral(subject)) return fragment.empty();

    // Load and cache the fragment
    var url = startFragment._fragmentsClient._startFragmentUrl;
    if (subject !== null) {
      url = url + '?s=' + encodeURIComponent(subject);
      if (pattern.length > 0)
        url = url + '&triples=' + pattern.length + '&star=' + encodeURIComponent(starStr);
    }
    else if (pattern.length > 0)
      url = url + '?triples=' + pattern.length + '&star=' + encodeURIComponent(starStr);
    // url = encodeURI(url);

    fragment.loadStarFromUrl(url);
  });
...
// Returns the Star Pattern Fragment for the given star pattern
FragmentsClient.prototype.getFragmentByStarPatternWithBindings = function (pattern, bindings) {
  if (pattern.length === 1)
    return this.getFragmentByTriplePatternWithBindings(pattern[0], bindings);

  var bindStr = rdf.findRelevantBindings(pattern, bindings);
  // Check whether the fragment was cached
  var cache = this._cache, key = JSON.stringify(pattern) + '&' + bindStr;
  if (cache.has(key))
    return cache.get(key).clone();
  // Create a dummy iterator until the fragment is loaded
  var fragment = new Fragment(this, pattern);
  ...
  startFragment.getProperty('controls', function (controls) {
    ...
    if (subject !== null) {
      ...
      url = url + '?triples=' + pattern.length + '&star=' + encodeURIComponent(starStr);
    if (bindStr !== '')
      url = url + '&values=' + (encodeURIComponent(bindStr));
    // url = encodeURI(url);
...
FragmentsClient.prototype.getFragmentByTriplePattern = function (pattern) {
  // Check whether the fragment was cached
  var cache = this._cache, key = JSON.stringify(pattern);
  if (cache.has(key))
    return cache.get(key).clone();
...
// Returns the Triple Pattern Fragment for the given triple pattern
FragmentsClient.prototype.getFragmentByTriplePatternWithBindings = function (pattern, bindings) {
  var bindStr = rdf.findRelevantBindings(pattern, bindings);
  // Check whether the fragment was cached
  var cache = this._cache, key = JSON.stringify(pattern) + '&' + bindStr;
  if (cache.has(key))
  ...
  startFragment.getProperty('controls', function (controls) {
    ...
    // Load and cache the fragment
    pattern = { subject: subject, predicate: predicate, object: object };
    var url = controls.getFragmentUrl(pattern);
    if (bindStr !== '')
      url = url + '&values=' + encodeURIComponent(bindStr);

    fragment.loadFromUrl(url);
  });
  cache.set(key, fragment);
  return fragment.clone();
};
...
Fragment.prototype.loadStarFromUrl = function (pageUrl) {
  // Fetch a page of the fragment
  var fragment = this, fragmentsClient = this._fragmentsClient, rawPage,
      headers = { 'user-agent': 'Star Pattern Fragments Client' };
