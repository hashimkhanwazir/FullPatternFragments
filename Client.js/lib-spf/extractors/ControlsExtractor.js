...
ControlsExtractor.prototype._extract = function (metadata, tripleStream, callback) {
  ...
  tripleStream.on('data', function (triple) {
    if (triple.star === undefined) {
      ...
      triple.star.forEach(function (tpl) {
        if (tpl.predicate.indexOf(rdf.HYDRA) === 0) {
          var property = tpl.predicate.substr(rdf.HYDRA.length),
              propertyData = controlData[property] || (controlData[property] = {}),
              subjectData = propertyData[tpl.subject] || (propertyData[tpl.subject] = []);
          subjectData.push(tpl.object);
        }
      });
    }
  });
  ...
  tripleStream.on('end', function () {
    ...
    if (searchForms) {
      ...
      controls.getFragmentUrlStar = function (starPattern) {
        var variables = {};
        variables.s   = starPattern.subject;
        variables.star = starPattern.star;
        // variables[mappings[rdf.RDF_OBJECT]]    = starPattern.object;
        return searchTemplate.expand(variables);
      };
    }
