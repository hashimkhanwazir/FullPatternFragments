...
//}
        ...
        try {
            ...
            if (request.getParameter("triples") == null) {
                ...
                TriplePatternFragment _fragment;
                if (bindings != null) {
                ...
                writeTpf(request, response, query, fragment, limit, offset, page);
            } else {
                ...
                if (bindings != null) {
                    _fragment = ((HdtDataSource) dataSource).getBindingFragment(star, offset,
                            limit, bindings, foundVariables);
                } else {
                    _fragment = ((HdtDataSource) dataSource).getFragment(star, offset, limit);
                }
                final StarPatternFragment fragment = _fragment;

                writeSpf(request, response, query, fragment, limit, offset, page);
            }
        } catch (Exception e) {
    ...
    private void writeSpf(HttpServletRequest request, HttpServletResponse response, String query,
                          StarPatternFragment fragment, long limit, long offset, long page) throws Exception {
        // fill the output model
        final List<Model> outputs = fragment.getStarPatterns();
        if(outputs.size() == 0) return;

        Model output = outputs.get(0);
        ...
        for (int i = 1; i < size; i++) {
            Model out = outputs.get(i);
            out.write(response.getWriter(), "Turtle");
        }
    }

    private void writeTpf(HttpServletRequest request, HttpServletResponse response, String query,
                          TriplePatternFragment fragment, long limit, long offset, long page) throws Exception {
        // fill the output model
     ...
     *
     * @param value with a string containing FILTER expressions
